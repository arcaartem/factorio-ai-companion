...
- **`resource_nearest` is not actually nearest** (T-034): it searches a ±200 square with `limit = 100`, so the engine's arbitrary first-100 slice can exclude the closest patch — probed returning ore at distance 155 while ore sat at 85.5. Don't build a test on it; plant your own arena ore.
