...
- Validation: `bun run scripts/validate-tools.ts` (**51 tools = 51 Lua commands as of mod 0.20.0**, which removed `action_wololo`; 0.15.0 removed `companion_realistic`; also checks arity/argument order, not just names). CAVEAT: it covers the request side only — Lua *response shapes* and the hand-rolled command strings inside `src/skills/*.ts` are unchecked, and both have drifted before. Contract changes need a live in-game check, not just a green validator. **`checkArity` now also flags an optional Lua capture that NO tool exposes (added 0.21.0)** — it used to assert only that the TS placeholder count fell within `[mandatory..total]`, so `fac_companion_inventory` declaring 1 of its 3 captures passed cleanly while its chest-inspection branch stayed unreachable through MCP. That check lit up 4 tools / 5 captures, all now exposed (`companion_inventory` x/y, `companion_health` target, `item_recipes` filter, `research_progress` technology), and it is a hard error — so adding a Lua capture without a TS param fails the pre-commit gate. The mirror-image defect is still invisible: `validate-tools.ts` skips the 17 commands with no `parse_args` at all, which is how `TOOLS.help` gets away with templating a `{category}` that `help.lua` never accepts. **Do NOT trust its "consistent arity/order" line to mean the ORDER is right — it cannot check order at all.** Lua captures are anonymous (`(%S+)`, `(%d*)`), so nothing connects capture 4 to the concept "x"; the check compares placeholder *counts* only. It reports all 38 pattern-backed tools consistent while `building_fill`'s template emits `{companionId} {x} {y} {itemName} {count} {entityName}` against a handler reading `id, item, count, x, y, entityName` — a live defect (T-058) where every coordinate-addressed call fails `parse_args` outright. Settle an order question by matching the wire string `buildRCONCommand` emits against the handler's own pattern, in Lua (`~/.local/share/mise/installs/lua/latest/bin/lua`), not by reading either side alone.
- **A setup pre-scan filtered more narrowly than the predicate it is warning about produces false
  confidence, not safety** — and a suite that asserts its target is *reachable* has not asserted it
  is the one that will be *chosen*. t019 section 1a placed its chest at `px+2`, asserted it sat
  inside `building_empty`'s radius-3 search, then called the tool with **no** `x`/`y` — leaving the
  subject to `u.resolve_target`'s nearest-match. An `assembling-machine-1` at 1.92 tiles beat the
  chest at 2.37, so 8 of 11 assertions ran against the wrong entity and reported defects that did
  not exist. The guard scan missed it by filtering `type={"container","logistic-container"}` and
  reporting `count: 0` while **five** qualifying entities stood within radius 6: anything with a
  `furnace_result`/`assembling_machine_output` inventory satisfies `empty`, so a container-typed
  scan cannot see its real competition. This is also why the bug reproduced on saves predating the
  science block — on any save some furnace or burner-drill from the original line sits nearer than
  the chest, which is what made "the new build moved the nearest entity" look decisive and be
  wrong. Two rules: pin the subject explicitly (pass the position `building_place`/`create_entity`
  actually returned, plus `entityName`) whenever the assertion is about something other than
  resolution, and write the guard scan with the *same* predicate the command uses.
